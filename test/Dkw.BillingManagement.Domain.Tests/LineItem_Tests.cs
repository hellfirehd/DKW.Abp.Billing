...
public class LineItem_Tests<TStartupModule> : BillingManagemenDomainTestBase<TStartupModule>
    where TStartupModule : IAbpModule
{
    ...
    public async Task LineItem_ShouldCalculateTotal_WithTax()
    {
        ...
        var lineItem = await TaxableProductItemAsync(EffectiveDate);
    ...
    [Fact]
    public async Task LineItem_ShouldCalculateTotal_WithExemptions()
    {
        // Arrange
        var tax = new Tax(Guid.NewGuid(), "GST", "GST").AddTaxRate(0.05m, new DateOnly(2000, 01, 01)); // 5% GST
